...
?><!DOCTYPE html>
<html <?php language_attributes(); ?> <?php if(isset($dotlife_homepage_style) && !empty($dotlife_homepage_style)) { echo 'data-style="'.esc_attr($dotlife_homepage_style).'"'; } ?> data-menu="<?php echo esc_attr($tg_menu_layout); ?>">
<head>
...
<body <?php body_class(); ?>>
	...
	<?php
		...
		if(DOTLIFE_THEMEDEMO && isset($_GET['topbar']) && !empty($_GET['topbar']))
		{
			$tg_topbar = true;
		}
	...
	<input type="hidden" id="pp_back" name="pp_back" value="<?php esc_html_e('Back', 'dotlife' ); ?>"/>
	...
	<?php
		...
		{
			$woocommerce = dotlife_get_woocommerce();
	...
	?>
	<div id="wrapper" class="<?php if(!empty($dotlife_page_menu_transparent)) { ?>hasbg<?php } ?> <?php if(!empty($page_menu_transparent)) { ?>transparent<?php } ?>">
