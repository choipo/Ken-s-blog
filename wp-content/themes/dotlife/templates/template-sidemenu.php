...
<div class="mobile_menu_wrapper">
    ...
    <?php
    	...
    	if(class_exists('Woocommerce') && dotlife_is_woocommerce_page())
    	{
    	...
    	if($dotlife_homepage_style == 'fullscreen')
        {
    	    ...
    	    <a class="logo_wrapper <?php if(!empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
    	    	<?php
					...
					<?php
						...
						else if($image_width == 1 && $image_height == 1 && $tg_retina_logo != dotlife_get_demo_logo('tg_retina_logo'))
						{
					?>
	    	    	...
	    	    	<?php 
						...
						{
					?>
	    	    	<img src="<?php echo esc_url($tg_retina_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="75" height="51"/>
	    	    	<?php 
		    	    	}
		    	    ?>
    	    </a>
	...
	else
	{
        ...
        {
        	//Get image width and height
	    	$image_id = dotlife_get_image_id($tg_retina_transparent_logo);
	    	$obj_image = wp_get_attachment_image_src($image_id, 'original');
	    	$image_width = 0;
	    	$image_height = 0;
    ...
    <?php
			...
			<div class="header_client_wrapper">
			    <a class="client_login_link" href="javascript:;" title="<?php esc_html_e('Client Login', 'dotlife' ); ?>"><span class="ti-lock"></span><?php esc_html_e('Client Login', 'dotlife' ); ?></a>
			</div>
			<?php
				}
			...
			?>
			<div class="header_client_wrapper">
			    <span class="ti-user"></span><a href="<?php echo esc_url($user_home_url); ?>"><?php echo esc_html($current_user->display_name); ?></a>&nbsp;(<a class="client_logout_link" href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php esc_html_e('Logout', 'dotlife' ); ?>"><?php esc_html_e('Logout', 'dotlife' ); ?></a>)
			</div>
	<?php
				}
			}
...
<?php
	$dotlife_page_menu_transparent = dotlife_get_page_menu_transparent();
	dotlife_set_page_menu_transparent($page_menu_transparent);
?>
<!-- End mobile menu -->